                          ...
                          PaStreamParameters &params, double &sampleRate)
{
  ...
  case 32:
    ...
    break;
  default:
    throw Napi::Error::New(env, "Invalid sampleFormat");
  }

  params.suggestedLatency = isInput ? Pa_GetDeviceInfo(params.device)->defaultLowInputLatency : Pa_GetDeviceInfo(params.device)->defaultLowOutputLatency;
  params.hostApiSpecificStreamInfo = NULL;

  sampleRate = (double)options->sampleRate();

#ifdef __arm__
  params.suggestedLatency = isInput ? Pa_GetDeviceInfo(params.device)->defaultHighInputLatency : Pa_GetDeviceInfo(params.device)->defaultHighOutputLatency;
#endif
}
